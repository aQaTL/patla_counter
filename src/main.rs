...
async fn fetch_all_rows(_req: HttpRequest, pool: web::Data<Pool>, id: Identity) -> Result<impl Responder, Error> {
...
async fn add(form: web::Form<AddForm>, pool: web::Data<Pool>, id: Identity) -> Result<impl Responder, Error> {
    if let None = id.identity() {
        return Ok(HttpResponse::Forbidden().body("Access denied"));
    }
    ...
    web::block(move || {
        use self::schema::entries::dsl::*;
        let conn = pool.get().unwrap();
        diesel::insert_into(entries).values(&new_entry).execute(&conn)
    })
    .await
    .map_err(|_| HttpResponse::InternalServerError())?; // convert diesel error to http response
...
async fn auth(form: web::Form<AuthForm>, pool: web::Data<Pool>, id: Identity) -> Result<impl Responder, Error> {
